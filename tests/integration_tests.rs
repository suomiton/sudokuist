...
const VALID_PUZZLE: [Option<u8>; 81] = [
    Some(5), Some(3), None,    Some(6), None,    Some(8), None,    Some(1), Some(2),
    Some(6), None,    Some(2), Some(1), Some(9), Some(5), None,    Some(4), None,
    Some(1), Some(9), Some(8), None,    Some(4), Some(2), Some(5), None,    Some(7),
    Some(8), Some(5), Some(9), Some(7), Some(6), None,    Some(4), Some(2), Some(3),
    Some(4), None,    Some(6), Some(8), Some(5), Some(3), Some(7), None,    Some(1),
    Some(7), Some(1), Some(3), Some(9), None,    Some(4), Some(8), Some(5), Some(6),
    Some(9), Some(6), Some(1), Some(5), Some(3), Some(7), None,    Some(8), Some(4),
    Some(2), Some(8), None,    Some(4), Some(1), Some(9), Some(6), Some(3), Some(5),
    Some(3), Some(4), Some(5), None,    Some(8), Some(6), Some(1), Some(7), None
];

/// An invalid board with conflicts
const INVALID_BOARD: [Option<u8>; 81] = [
    Some(5), Some(5), None,    Some(6), None,    Some(8), None,    Some(1), Some(2), // Row conflict: two 5s
    Some(6), None,    Some(2), Some(1), Some(9), Some(5), None,    Some(4), None,
    Some(1), Some(9), Some(8), None,    Some(4), Some(2), Some(5), None,    Some(7),
    Some(8), Some(5), Some(9), Some(7), Some(6), None,    Some(4), Some(2), Some(3),
    Some(4), None,    Some(6), Some(8), Some(5), Some(3), Some(7), None,    Some(1),
...
mod validation_tests {
    ...
    fn test_valid_complete_board() {
        ...
        assert!(result.invalid_indices.is_empty(), "Valid complete board should have no invalid indices");
        assert!(result.is_complete, "Complete valid board should be marked as complete");
    }

    #[test]
    fn test_valid_incomplete_board() {
        let result = validate_board(&VALID_PUZZLE);
        assert!(result.invalid_indices.is_empty(), "Valid puzzle should have no invalid indices");
        assert!(!result.is_complete, "Incomplete board should not be marked as complete");
    }

    #[test]
    fn test_invalid_board_detected() {
        let result = validate_board(&INVALID_BOARD);
        assert!(!result.invalid_indices.is_empty(), "Invalid board should have invalid indices detected");
        assert!(!result.is_complete, "Invalid board should not be complete");
        
        // The conflict is at index 1 (second cell in first row)
        assert!(result.invalid_indices.contains(&1), "Should detect the duplicate 5 in first row");
    }
    ...
    fn test_empty_board() {
        ...
        assert!(result.invalid_indices.is_empty(), "Empty board should be valid");
...
mod solver_tests {
    ...
    fn test_solver_creation() {
        ...
        assert!(!solver.is_solved(), "Incomplete puzzle should not be marked as solved");
    }

    #[test]
    fn test_solver_with_complete_board() {
        let solver = HumanStyleSolver::new(&VALID_COMPLETE_BOARD);
        assert!(solver.is_solved(), "Complete valid board should be marked as solved");
    }
    ...
    fn test_solver_techniques() {
        ...
        assert!(solved || !initial_solved, "Solver should either solve or make progress");
        
        // Check that some techniques were used
        let techniques = solver.get_techniques_used();
        assert!(!techniques.is_empty(), "Some solving techniques should have been used");
    }
    ...
    fn test_branching_factor() {
        ...
        assert!(factor >= 1.0 && factor <= 9.0, "Branching factor should be between 1 and 9");
    }
}
...
mod generator_tests {
    ...
    fn test_generate_easy_puzzle() {
        ...
        if let Some(puzzle) = generator.generate() {
            ...
            assert!(result.invalid_indices.is_empty(), "Generated puzzle should be valid");
            
            // Check it has a reasonable number of clues
            let clue_count = puzzle.iter().filter(|&&cell| cell.is_some()).count();
            assert!(clue_count >= 17 && clue_count <= 50, "Puzzle should have reasonable number of clues");
            
            // Check it has a unique solution
            assert!(has_unique_solution(&puzzle), "Generated puzzle should have unique solution");
        }
    }
    ...
    fn test_generate_medium_puzzle() {
        ...
        if let Some(puzzle) = generator.generate() {
            let result = validate_board(&puzzle);
            assert!(result.invalid_indices.is_empty(), "Generated medium puzzle should be valid");
            assert!(has_unique_solution(&puzzle), "Generated puzzle should have unique solution");
        }
    }
    ...
    fn test_generator_configuration() {
        ...
        assert_eq!(std::mem::size_of_val(&generator), std::mem::size_of::<PuzzleGenerator>());
    }
}
...
mod difficulty_tests {
    ...
    fn test_difficulty_analysis() {
        ...
        assert!(matches!(analysis.level, DifficultyLevel::Easy | DifficultyLevel::Medium | DifficultyLevel::Hard | DifficultyLevel::Expert));
        assert!(analysis.technique_diversity <= 10, "Technique diversity should be reasonable");
        assert!(analysis.branching_factor >= 1.0 && analysis.branching_factor <= 9.0, "Branching factor should be reasonable");
    }
    ...
    fn test_empty_board_difficulty() {
        ...
        assert!(analysis.branching_factor > 5.0, "Empty board should have high branching factor");
    }
}
...
mod integration_tests {
    ...
    fn test_full_workflow() {
        ...
        if let Some(puzzle) = generator.generate() {
            ...
            assert!(validation.invalid_indices.is_empty(), "Generated puzzle should be valid");
            ...
            assert_eq!(analysis.level, DifficultyLevel::Easy, "Should match target difficulty");
            
            println!("Workflow test successful: solved={}, difficulty={:?}", solved, analysis.level);
        } else {
    ...
    fn test_workflow_with_known_puzzle() {
        ...
        // Step 1: Validate it
        let validation = validate_board(&puzzle);
        assert!(validation.invalid_indices.is_empty(), "Predefined puzzle should be valid");
        ...
        assert!(matches!(analysis.level, 
            DifficultyLevel::Easy | DifficultyLevel::Medium | 
            DifficultyLevel::Hard | DifficultyLevel::Expert
        ));
        
        println!("Workflow test with known puzzle successful: difficulty={:?}", analysis.level);
    }
    ...
    fn test_backtracking_solver() {
        ...
        let validation = validate_board(&puzzle);
        assert!(validation.is_complete && validation.invalid_indices.is_empty(), 
...
mod stress_tests {
    ...
    fn test_multiple_generations() {
        ...
        for i in 0..attempts {
            if let Some(puzzle) = generator.generate() {
                let validation = validate_board(&puzzle);
                assert!(validation.invalid_indices.is_empty(), 
                    "Generated puzzle {} should be valid", i);
                success_count += 1;
            }
        }
        
        println!("Successfully generated {}/{} puzzles", success_count, attempts);
